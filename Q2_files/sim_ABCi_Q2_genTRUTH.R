###############################################################
#Generate counterfactual data for Q2. 
#This files generates data as if everyone were to be never treated from the start of follow-up (the never treat strategy).
#See the file sim_ABCi_Q1_genTRUTH file to generates data as if everyone starting treatment were to start treatment A, B, C or D.
#The treatment is specified by trt.fix in analysis_Q2_TRUTH.R
#
#Data up to treatment initiation are generated in the same way as in the 'observed' data (sim_ABCi_data_generate.R).
...
#set these to be those of the treatment of interest: in this case set them all to zero, to represent the never treat strategy
#--------------------------------------------

#hazard of MACE: main effects of treatment (log HRs)
coef.mace.A<-coef.mace.B<-coef.mace.C<-coef.mace.D<-0

#on hazard of MACE: interactions with cvd and kidney disease
coef.mace.A.cvd<-coef.mace.B.cvd<-coef.mace.C.cvd<-coef.mace.D.cvd<-0
coef.mace.A.kidney<-coef.mace.B.kidney<-coef.mace.C.kidney<-coef.mace.D.kidney<-0

#hazard of death: main effects of treatment (log HRs)
coef.death.A<-coef.death.B<-coef.death.C<-coef.death.D<-0

#bmi
bmi.coef.A.yr1<-bmi.coef.B.yr1<-bmi.coef.C.yr1<-bmi.coef.D.yr1<-0
bmi.coef.A.longterm<-bmi.coef.B.longterm<-bmi.coef.C.longterm<-bmi.coef.D.longterm<-0

#hba1c
hb.slope.A.yr1<-hb.slope.B.yr1<-hb.slope.C.yr1<-hb.slope.D.yr1<-0
hb.slope.A.yr1to2<-hb.slope.B.yr1to2<-hb.slope.C.yr1to2<-hb.slope.D.yr1to2<-0

hb.const.A.yr1to2<-hb.const.B.yr1to2<-hb.const.C.yr1to2<-hb.const.D.yr1to2<-0
hb.const.A.yr2plus<-hb.const.B.yr2plus<-hb.const.C.yr2plus<-hb.const.D.yr2plus<-0

#kidney disease
kid.coef.A<-kid.coef.B<-kid.coef.C<-kid.coef.D<-0

#--------------------------------------------
#--------------------------------------------
#FROM THIS POINT ONWARDS THE DATA GENERATION CODE IS AS IN sim_ABCi_data_generate.R
#--------------------------------------------
#--------------------------------------------
...
#---
#save data
#The data set is too big to be stored in standard github, so it is broken into two parts for storage here
dta_part1<-dta[1:floor(dim(dta)[1]/2),]
dta_part2<-dta[(floor(dim(dta)[1]/2)+1):dim(dta)[1],]
data_ABCi_Q2_counterfactual_nevertrt_part1<-dta_part1
data_ABCi_Q2_counterfactual_nevertrt_part2<-dta_part2

save(data_ABCi_Q2_counterfactual_nevertrt_part1,file="data_ABCi_Q2_counterfactual_nevertrt_part1.rds")
save(data_ABCi_Q2_counterfactual_nevertrt_part2,file="data_ABCi_Q2_counterfactual_nevertrt_part2.rds")
